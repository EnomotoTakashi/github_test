...
def fn_ccv(r0, r1, c1, r2, c2, soc_ary, t0_ary, t1_ary, current):
    ocv = fn_sococv(soc_ary)
    v0 = r0 * current * t0_ary
    v1 = r1 * current * (1 - np.exp(-t1_ary / r1 / c1))
    v2 = r2 * current * (1 - np.exp(-t1_ary / r2 / c2))
    ccv = ocv + v0 + v1 + v2
    return ccv
...
def rc_fitting(x, xdata, ydata, current, soc_ini):
    ...
    t0_ary[0] = 0.0
    ccv_ary = fn_ccv(r0, r1, c1, r2, c2, soc_ary, t0_ary, t1_ary, current)
    res = ydata - ccv_ary
    return res
...
x0 = np.array([0.01, 0.01, 1000, 0.001, 10])
res = least_squares(rc_fitting, x0, args=(xdata, ydata, current, soc_ini), bounds=((0, 0, 0, 0, 0), (10, 10, 10000, 10, 10000)))
r0, r1, c1, r2, c2 = res.x
t1_ary = np.arange(0, 30.1, 0.1)
t0_ary = np.ones(len(t1_ary))
t0_ary[0] = 0.0
soc_ary = soc_ini + current * t1_ary / 3600 / 3.0
ccv = fn_ccv(r0, r1, c1, r2, c2, soc_ary, t0_ary, t1_ary, current)
plt.plot(xdata, ydata, "o")
plt.plot(t1_ary, ccv)
plt.show()
print(r0, r1, c1, r2, c2)
