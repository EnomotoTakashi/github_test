...
SOC_MIN, SOC_MAX = 0.1, 0.8
COEFFICIENTS = np.array([-1.2918e+00, 9.6896e+00, -2.1023e+01, 1.9749e+01, -8.0028e+00, 1.6362e+00, 3.4722e+00])

def fn_soc_ocv(soc):
    return sum(coefficient * soc ** power for power, coefficient in enumerate(COEFFICIENTS[::-1]))

def validate_soc(soc):
    return np.clip(soc, SOC_MIN, SOC_MAX)

def fn(soc, soc_ary, ary):
    soc = validate_soc(soc)
    f = interp1d(soc_ary, ary, kind="cubic")
    return f(soc)
...
def calc_v1(soc, current, dt, v1):
    r1 = fn_r1(soc)
    c1 = fn_c1(soc)
    fv1 = lambda v, t: current / c1 - v / r1 / c1
